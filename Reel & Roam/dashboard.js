...
// Place card interaction function
function togglePlaceInfo(placeId) {
    const placeCard = document.querySelector(`[onclick="togglePlaceInfo('${placeId}')"]`);
    const placeInfo = document.getElementById(`place-info-${placeId}`);
    const expandIcon = placeCard?.querySelector('.place-expand-icon i');

    if (!placeCard || !placeInfo) return;

    const isExpanded = placeCard.getAttribute('aria-expanded') === 'true';

    // Close all other place info sections
    ...
    document.querySelectorAll('.place-card').forEach(card => {
        ...
        const icon = card.querySelector('.place-expand-icon i');
        if (icon) {
    ...
    if (!isExpanded) {
        // Open this place info
        placeInfo.hidden = false;
        placeCard.setAttribute('aria-expanded', 'true');
        if (expandIcon) {
...
// Download Trip Plans Functionality
const tripData = [
    {
        id: 'trip-1',
        ...
        groupSize: 2,
        status: 'completed',
        image: 'https://images.unsplash.com/photo-1502602898534-37dd6d3935d9?w=400&h=400&fit=crop&crop=center',
        ...
        tips: [
            ...
            'Eat at local bistros instead of tourist restaurants',
            'Stay in arrondissements 11-20 for better prices'
        ],
        bookingLinks: {
            flights: 'https://www.goindigo.in/',
            trains: 'https://www.irctc.co.in/nget/train-search',
            hotels: 'https://www.airbnb.co.in/',
            internationalRail: 'https://uic.org/'
        }
    },
    {
        id: 'trip-2',
        ...
        tips: [
            ...
            'Visit during cherry blossom season for free views'
        ],
        bookingLinks: {
            flights: 'https://www.goindigo.in/',
            trains: 'https://www.irctc.co.in/nget/train-search',
    ...
    },
    {
        ...
        groupSize: 3,
        status: 'upcoming',
        image: 'https://images.unsplash.com/photo-1537953773345-d172ccf13cf1?w=400&h=400&fit=crop&crop=center',
        lastUpdated: '2024-05-25T09:45:00Z',
        budgetBreakdown: {
            accommodation: 16250,
            food: 13000,
            transport: 9750,
        ...
        tips: [
            ...
            'Eat at local warungs (small restaurants)',
            'Rent a scooter for transportation',
            'Visit during shoulder season (Apr-Jun, Sep-Nov)'
        ],
        bookingLinks: {
            flights: 'https://www.goindigo.in/',
            trains: 'https://www.irctc.co.in/nget/train-search',
...
function initializeDownloadSection() {
    renderDownloadCards();
}
...
function renderDownloadCards() {
    ...
    if (tripData.length === 0) {
        container.innerHTML = `
            <div class="download-empty-state">
                ...
                <h3>No trips to download</h3>
                <p>Plan your first trip to generate downloadable summaries</p>
                <button class="btn-primary" onclick="showSection('plan-trip')">
    ...
    container.innerHTML = tripData.map(trip => createDownloadCard(trip)).join('');
}

function createDownloadCard(trip) {
    const statusClass = trip.status === 'completed' ? 'completed' :
        trip.status === 'ongoing' ? 'ongoing' : 'upcoming';

    const statusText = trip.status === 'completed' ? 'Completed' :
        trip.status === 'ongoing' ? 'Ongoing' : 'Upcoming';

    const lastUpdated = new Date(trip.lastUpdated).toLocaleDateString('en-IN', {
        year: 'numeric',
        month: 'short',
        day: 'numeric',
        hour: '2-digit',
    ...
    return `
        <div class="download-trip-card" data-trip-id="${trip.id}">
            <div class="download-trip-image">
                <img src="${trip.image}" alt="${trip.destination}" />
            </div>
            <div class="download-trip-content">
                <div class="download-trip-title">
                    <i class="fas fa-map-marker-alt"></i>
                    ${trip.title}
                    <span class="download-trip-status ${statusClass}">${statusText}</span>
                </div>
                <div class="download-trip-destination">${trip.destination}</div>
                <div class="download-trip-details">
                    <div class="download-trip-detail">
                    ...
                    <div class="download-trip-detail">
                        ...
                        <span>${trip.groupSize} person${trip.groupSize > 1 ? 's' : ''}</span>
                    </div>
                    <div class="download-trip-detail">
            ...
            <div class="download-trip-actions">
                <button class="download-action-btn download" 
                        title="Download PDF" 
...
function generateTripPDF(tripId) {
    ...
    // Generate professional PDF using jsPDF with exact business card design
    setTimeout(() => {
        try {
            const { jsPDF } = window.jspdf;
            const doc = new jsPDF();

            // Color scheme exactly matching the business card template
            const tealColor = [0, 128, 128]; // Deep teal like the card
            const lightTeal = [0, 150, 150]; // Lighter teal for patterns
            const whiteColor = [255, 255, 255];
            const darkText = [59, 73, 72];
            const lightText = [128, 128, 128];

            // Page 1: Front side (like business card front)
            // Create teal header background (like card front)
            doc.setFillColor(...tealColor);
            doc.rect(0, 0, 210, 80, 'F');

            // Add diagonal patterns like the business card
            doc.setFillColor(...lightTeal);
            for (let i = 0; i < 8; i++) {
                doc.rect(5 + (i * 12), 8, 6, 2, 'F');
                doc.rect(10 + (i * 12), 18, 8, 2, 'F');
                doc.rect(8 + (i * 12), 28, 5, 2, 'F');
                doc.rect(12 + (i * 12), 38, 7, 2, 'F');
                doc.rect(6 + (i * 12), 48, 9, 2, 'F');
                doc.rect(11 + (i * 12), 58, 4, 2, 'F');
            }

            // Logo/Icon (centered like business card)
            doc.setTextColor(...whiteColor);
            doc.setFontSize(28);
            doc.text('✈️', 105, 30, { align: 'center' });

            // Company name (like ARTIFACT ADEPT on the card)
            doc.setFontSize(20);
            doc.text('REEL \'N ROME', 105, 45, { align: 'center' });

            // Subtitle (like "Museum" on the card)
            doc.setFontSize(14);
            doc.text('Travel Dashboard', 105, 58, { align: 'center' });

            // Trip title
            doc.setFontSize(16);
            doc.text(trip.title, 105, 72, { align: 'center' });

            // Curved divider (like the organic shape on card back)
            doc.setFillColor(...whiteColor);
            doc.rect(0, 75, 210, 25, 'F');

            // Add curved effect
            doc.setFillColor(245, 245, 245);
            doc.rect(0, 75, 210, 8, 'F');

            // Main content area (white background like card back)
            doc.setFillColor(...whiteColor);
            doc.rect(0, 100, 210, 110, 'F');

            // Contact info section (like business card back)
            doc.setTextColor(...darkText);
            doc.setFontSize(16);
            doc.text('Trip Summary', 105, 115, { align: 'center' });

            doc.setFontSize(12);
            doc.text(`Destination: ${trip.destination}`, 20, 130);
            doc.text(`Duration: ${trip.duration} days`, 20, 140);
            doc.text(`Group Size: ${trip.groupSize} person${trip.groupSize > 1 ? 's' : ''}`, 20, 150);
            doc.text(`Dates: ${trip.startDate} to ${trip.endDate}`, 20, 160);

            // Budget info
            const totalBudget = trip.budget;
            const perPerson = totalBudget / trip.groupSize;
            const perDay = totalBudget / trip.duration;

            doc.setFontSize(14);
            doc.text('Budget Summary', 20, 175);

            doc.setFontSize(10);
            doc.text(`Total: ₹${totalBudget.toLocaleString('en-IN')}`, 20, 185);
            doc.text(`Per Person: ₹${perPerson.toLocaleString('en-IN')}`, 20, 195);
            doc.text(`Per Day: ₹${perDay.toLocaleString('en-IN')}`, 20, 205);

            // Page 2: Detailed information
            doc.addPage();

            // Header for page 2 (teal background)
            doc.setFillColor(...tealColor);
            doc.rect(0, 0, 210, 40, 'F');

            doc.setTextColor(...whiteColor);
            doc.setFontSize(18);
            doc.text('Travel Details', 105, 25, { align: 'center' });
            ...
            const budgetData = [
                ['Category', 'Amount (₹)'],
                ['Accommodation', trip.budgetBreakdown.accommodation.toLocaleString('en-IN')],
                ['Food & Drinks', trip.budgetBreakdown.food.toLocaleString('en-IN')],
                ['Transportation', trip.budgetBreakdown.transport.toLocaleString('en-IN')],
                ['Activities', trip.budgetBreakdown.activities.toLocaleString('en-IN')],
                ['Emergency Fund', trip.budgetBreakdown.emergency.toLocaleString('en-IN')]
            ];
            ...
            // Attractions section
            const attractionsY = doc.lastAutoTable.finalY + 20;
            doc.setFontSize(14);
            doc.text('Must-See Attractions', 20, attractionsY);

            doc.setFontSize(10);
            trip.attractions.forEach((attraction, index) => {
                doc.text(`📍 ${attraction}`, 20, attractionsY + 10 + (index * 8));
            });

            // Travel tips section
            const tipsY = attractionsY + 10 + (trip.attractions.length * 8) + 15;
            doc.setFontSize(14);
            doc.text('Travel Tips', 20, tipsY);

            doc.setFontSize(10);
            trip.tips.forEach((tip, index) => {
                const lines = doc.splitTextToSize(`💡 ${tip}`, 170);
                lines.forEach((line, lineIndex) => {
                    doc.text(line, 20, tipsY + 10 + (index * 12) + (lineIndex * 5));
                });
            });

            // Booking links section
            const bookingY = tipsY + 10 + (trip.tips.length * 12) + 20;
            doc.setFontSize(14);
            doc.text('Booking Links', 20, bookingY);

            doc.setFontSize(10);
            doc.text(`✈️ Flights: ${trip.bookingLinks.flights}`, 20, bookingY + 10);
            doc.text(`🚂 Trains: ${trip.bookingLinks.trains}`, 20, bookingY + 20);
            doc.text(`🏨 Hotels: ${trip.bookingLinks.hotels}`, 20, bookingY + 30);
            doc.text(`🌍 International: ${trip.bookingLinks.internationalRail}`, 20, bookingY + 40);

            // Contact section (like business card contact info)
            const contactY = bookingY + 50;
            doc.setFontSize(14);
            doc.text('Contact Information', 20, contactY);

            doc.setFontSize(10);
            doc.text('📧 Email: support@reelnrome.com', 20, contactY + 10);
            doc.text('📱 App: Reel \'N Rome Travel Dashboard', 20, contactY + 20);
            doc.text('🌐 Website: www.reelnrome.com', 20, contactY + 30);

            // Footer (like business card footer)
            doc.setFontSize(8);
            doc.setTextColor(...lightText);
            doc.text('Generated by Reel \'N Rome Travel Dashboard', 105, 280, { align: 'center' });
            doc.text(`Last updated: ${new Date(trip.lastUpdated).toLocaleDateString('en-IN')}`, 105, 285, { align: 'center' });

            // Save the PDF
            const filename = `${trip.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_trip_summary.pdf`;
            doc.save(filename);

            showLoadingOverlay(false);
            showNotification(`PDF for ${trip.title} generated successfully!`, 'success');
...
function shareTrip(tripId) {
    ...
    setTimeout(() => {
        try {
            ...
            // Generate the same PDF as in generateTripPDF function
            // Color scheme exactly matching the business card template
            const tealColor = [0, 128, 128];
            const lightTeal = [0, 150, 150];
            const whiteColor = [255, 255, 255];
            const darkText = [59, 73, 72];
            const lightText = [128, 128, 128];
            ...
            // Show share modal with PDF file
            const shareText = `Check out my ${trip.title} trip to ${trip.destination}! Generated by Reel 'N Rome Travel Dashboard.`;
            showShareModal(trip, shareText, pdfUrl, pdfBlob);

        } catch (error) {
            showLoadingOverlay(false);
            showNotification('Error generating PDF for sharing. Please try again.', 'error');
            console.error('PDF generation error:', error);
        }
    }, 1000);
}

function showShareModal(trip, shareText, pdfUrl, pdfBlob) {
    // Create modal content
    const modalContent = `
        <div class="modal-header">
            <h3>Share Trip Summary PDF</h3>
        ...
        <div class="modal-body">
            <div class="share-preview">
                ...
                <p>${trip.duration} days • ₹${trip.budget.toLocaleString('en-IN')} budget</p>
                <p class="pdf-info">📄 Professional PDF ready to share</p>
            </div>
            <div class="share-options">
                <button class="share-btn whatsapp" onclick="sharePDFToWhatsApp('${shareText}', '${pdfUrl}')">
                    <i class="fab fa-whatsapp"></i>
                    <span>WhatsApp PDF</span>
                </button>
                <button class="share-btn telegram" onclick="sharePDFToTelegram('${shareText}', '${pdfUrl}')">
                    <i class="fab fa-telegram"></i>
                    <span>Telegram PDF</span>
                </button>
                <button class="share-btn email" onclick="sharePDFViaEmail('${shareText}', '${pdfUrl}')">
                    <i class="fas fa-envelope"></i>
                    <span>Email PDF</span>
                </button>
                <button class="share-btn download" onclick="downloadPDF('${trip.title}', '${pdfUrl}')">
                    <i class="fas fa-download"></i>
                    <span>Download PDF</span>
                </button>
                <button class="share-btn copy" onclick="copyPDFLink('${shareText}', '${pdfUrl}')">
...
function sharePDFToWhatsApp(text, pdfUrl) {
    // For WhatsApp, we'll share the text with a note about the PDF
    const shareText = `${text}\n\n📄 PDF Trip Summary available for download.`;
    const url = `https://wa.me/?text=${encodeURIComponent(shareText)}`;
    window.open(url, '_blank');
    closeShareModal();
    showNotification('PDF download link shared to WhatsApp!', 'success');
}

function sharePDFToTelegram(text, pdfUrl) {
    // For Telegram, we'll share the text with PDF info
    const shareText = `${text}\n\n📄 PDF Trip Summary ready for sharing.`;
    const url = `https://t.me/share/url?url=${encodeURIComponent(window.location.href)}&text=${encodeURIComponent(shareText)}`;
    window.open(url, '_blank');
    closeShareModal();
    showNotification('PDF download link shared to Telegram!', 'success');
}

function sharePDFViaEmail(text, pdfUrl) {
    const subject = `Trip Summary: ${text.split('!')[0]}`;
    const body = `${text}\n\n📄 PDF Trip Summary attached.\n\nGenerated by Reel 'N Rome Travel Dashboard`;
    const mailtoUrl = `mailto:?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    window.open(mailtoUrl, '_blank');
    closeShareModal();
    showNotification('Email client opened with PDF info!', 'success');
}

function downloadPDF(tripTitle, pdfUrl) {
    // Create a download link for the PDF
    const link = document.createElement('a');
    link.href = pdfUrl;
    link.download = `${tripTitle.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_trip_summary.pdf`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    closeShareModal();
...
function copyPDFLink(text, pdfUrl) {
    const fullText = `${text}\n\n📄 PDF Trip Summary: ${pdfUrl}`;
...
function copyTripLink(text, url) {
    const fullText = `${text}\n\n${url}`;
    navigator.clipboard.writeText(fullText).then(() => {
...
function generateAndSharePDF(tripId) {
